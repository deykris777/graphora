...
export const Sidebar = () => {
    return (_jsxs("aside", { className: "sticky top-0 h-screen w-56 flex-shrink-0 px-5 py-8 flex flex-col transition-colors duration-300", style: { backgroundColor: 'var(--bg-base)', borderRight: '1px solid var(--border)' }, children: [_jsxs("div", { className: "flex items-center gap-3 px-2 mb-10", children: [_jsx("div", { className: "h-6 w-6 rounded flex items-center justify-center flex-shrink-0", style: { backgroundColor: 'var(--accent)' }, children: _jsx("span", { className: "text-xs font-black text-black", children: "G" }) }), _jsx("span", { className: "text-sm font-bold tracking-widest uppercase font-mono", style: { color: 'var(--text-heading)' }, children: "Graphora" })] }), _jsx("nav", { className: "flex-1 space-y-1.5", children: navItems.map((item) => (_jsxs(NavLink, { to: item.to, className: ({ isActive }) => `group flex items-center justify-between rounded-md px-3 py-2 text-[12px] font-medium tracking-wide transition-all duration-200`, style: ({ isActive }) => isActive ? {
